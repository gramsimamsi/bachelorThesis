...
\begin{itemize}
	\item Der Leerraum nach eine Punkt am Satzende ist größer als der nach einer Abkürzung (wie evtl.). Um Latex zu sagen, dass der Punkt kein Satzende ist, wird dieser bei Abkürzungen durch einen Backslash markiert, also so: evtl.\textbackslash, ohne Leerzeichen zwischen Punkt und Backslash.
	\item Leerräume zwischen zwei Teilen einer Abkürzung (wie z.\,B.) sind kleiner als zwischen einzelnen Wörtern, außerdem sollten diese beiden Buchstaben nicht getrennt werden. Dies erreicht man durch: z.\textbackslash,B.\textbackslash, wieder ohne Leerzeichen.
\end{itemize}
...
\begin{itemize}
	\item entweder bei Problemfällen manuelle Trennpunkte durch \textbackslash - in das Wort einfügen,
...
%\bigskip
%\noindent
Das \autoref{lst:bsp-bash-script} zeigt ein Beispiel für ein Bash Skript und das \autoref{lst:bsp-c-code-src} zeigt ein Beispiel für ein C-Code von einer Quelldatei.

\begin{lstlisting}[language={bash},
	style=custombash,
	caption={Beispiel Bash Skript},
	label={lst:bsp-bash-script}
]
#!/bin/bash
YEAR="2018"
echo "Hello world in ${YEAR}"
exit 0
\end{lstlisting}
...
\lstinputlisting[language={[ANSI]C},
	...
	firstline=1,
	lastline=6,
	caption={Beispiel C-Code from source file},
	label={lst:bsp-c-code-src}
]{\srcdir/hello_world.c}

\minisec{Tabelle in Textbreite (\emph{tabularx}):}
\begin{center}
\begin{tabularx}{\textwidth}{ >{\itshape}r X >{\bfseries}l}
   \toprule
   Position & Beschreibung & Anzahl \\
   \midrule
   1 & \emph{tabularx} bietet den Spaltentyp \emph{X} der iterativ eine
   automatische Spaltenverbreiterung durchführt, bis die gewünschte
   Gesamtbreite der Tabelle erreicht wird. Leider ist dieser Prozess langsam.
   Selbst auf neuen Computern verlangsamt sich der Kompilier-Vorgang bei
   einigen Tabellen erheblich. & 1 \\
  \bottomrule
\end{tabularx}
\end{center}

\minisec{Tabelle in Textbreite (\emph{calc}):}
\begin{center}
\begin{tabular}{>{\itshape}>{\raggedleft}p{1.5cm}p{\textwidth-6\tabcolsep-3cm}>{\bfseries}p{1.5cm}}
   \toprule
   Position & Beschreibung & Anzahl \\
   \midrule
   1 & Tabellen in Textbreite können auch ohne Zusatzpaket erstellt werden.
