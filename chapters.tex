...
AKS (on May 3rd 2019) => k8s-version <= 1.13.5, but only 1.11.8 or 1.11.9!
...
Impact was taken as single value between 1 and 4 and estimated through None/Theoretical (0), Low/Intermediate-Step (1), Moderate/Single-Tenant-pwn (2), Severe/Multi-Tenant-pwn (3)

Probability was a bitch to define proberly. Therefore split into four factors: Required Access Level (RAL), Vantage Point, Detectability and Exploitability. Those initially had values between 0 though 3, but outlier values were defined for RAL and Vantage Point (in sync with existing assessment methodologies within HvS).
Required Access Level: cloud/infrastructure admin (0, since a rogue employee with super-admin can do whatever they want and this is about baseline security); single-cluster/system-admin (1); single-cluster/system user (2); read-only (3); unauthenticated (4)
