...
Finding a framework to properly conduct a risk assessment proved to be more difficult and time consuming than expected. The process and result are laid out in this section.
...
The formula for estimating risk values of the vectors identified in section \ref{vectorIdentify} was developed with the three scenarios in mind that were derived in section \ref{scopeLimit}.

\mybox{yellow}{
TODO: based on specific software versions and (default) settings 
}
...
\subsection{Process of deriving a formula}
...
The original factors used to estimate a value for the likelihood were prevalence, detectability and exploitability. This methodology was customized in order to be more applicable, since prevalence would have been difficult to estimate and compare for such relatively new systems. Instead of detectability, the factors 'vantage point' and 'required access level' were added in order to explicitly consider both the possible origin of attack vectors as well as the privileges needed to leverage them. In contrast to other estimations they will not influence the exploitability value, as the formula would may then put too much weight on these factors. The average value of these four factors lead to the resulting probability value. Customizing the factors is done to tailor, a practice encouraged by the \gls{owasp} Risk Rating Methodology\footcite[][, section 'Step 6: Customizing the Risk Rating Model']{riskRating}

\mybox{yellow}{
TODO: values not percentages, only numbers relational to other vectors. 
TODO: map numbers to values
TODO: make official formulas?
TODO: based on specific software versions and (default) settings - put this into 'Specification of environment factors'!
}
