...
To follow the OWASP Risk Rating Methodology\footcite{riskRating} down the line, we need to define our threat agents and group them when applicable\footcite{threatModeling}. Examining a list of threat actors published through SANS\footcite{sansThreatActors}, we will only exclude state-sponsored actors. This leaves us with cyber criminals, hacktivists, systems administrators, end users, executives and partners.
...
There are many \gls{k8s} distributions, many of which are certified kubernetes offerings, meaning they all comply to the same standards set by the Linux Foundation through the \gls{cncf}, which oversees the project.\footcite{certifiedK8s}
...
The computing resources used to run the cluster are pre-configured and provisioned \gls{vm}s in the Azure cloud, which do not need to be configured and run a container runtime based on Moby, a toolkit from which the Docker runtime is built.\footcite{dockerMoby} As many cloud services do, there are multiple integrations to other cloud services, in case of \gls{aks} including an option to integrate your Azure Active Directory to authenticate users.\footcite{aadAksAuth}

%TODO any context missed here?

\chapter{Deriving the attack surface}

Picking up the three scenarios of section \ref{scopeLimit}, we identify the possible attack vectors in this chapter to get an overview of the attack surface that is posed by \gls{k8s} systems.
...
derive process/evolution of risk rating model, introduce rating by threat actors + tools + vector + platform, explain problems / inaccuracies
...
derive \& explain values for each vector

\subsection{Risk estimation result}
%TODO
TODO results upfront, values will be derived in the following subsections

TODO split figure vertically (one AKS, one OCP)

\begin{figure}[H]
\includegraphics[scale=0.45]{pictures/RiskAssess.jpg} 
\caption{Result of the risk estimation pre security measures TODO}
\label{fig:estimateResult}
\end{figure}
...
explain what to do if doing everything, will only implement two examples (list all measures and post-measure risks in annex anyway)

\section{Managing the risk of V09 - Lateral movement from container}
...
dont allow privileged containers (scc in OCP, podsecuritypolicies in AKS)
...
use multitenant network plugin (ocp), networkpolicies (aks)
